...
void Solver::a_sigma_i_func(const Ind& ind, int inbr, Vector2* a_sigma_i_p, double* a_sigmap){
...
void Solver::update_coeff_inner_pair(const Ind& ind, int inbr){ // add coefficient from a inner (no-a-boundary) face  
  ...
  // calculate A_K
  a_sigma_i_func(ind, inbr, &a_sigma_i_K, &a_sigma_K);

  // calculate A_L
  m.get_nbr_ind(ind, inbr, &nbr_ind); 
  rinb = m.rinbr(inbr);
  a_sigma_i_func(nbr_ind, rinb, &a_sigma_i_L, &a_sigma_L); 
